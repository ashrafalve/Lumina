      ...
      <main className="flex-1 flex flex-col min-w-0 relative">
        {!state.selectedNoteId ? (
          <div className="flex-1 flex flex-col h-full">
              ...
              <div className="max-w-6xl mx-auto flex flex-col gap-6">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    ...
                    <h1 className="text-3xl font-black text-white tracking-tight">
                      {state.activeTag ? `#${state.activeTag}` : 'My Thoughts'}
                    </h1>
                  </div>
                  <div className="flex items-center gap-3">
                    <div className="hidden md:flex items-center gap-1 bg-white/5 rounded-full px-3 py-1 border border-white/5">
                      ...
                      <span className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Local Mode</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="relative group">
                  ...
                  <input
                    ...
                    placeholder="Search through your genius..."
                ...
                {pinnedNotes.length > 0 && (
                  <section>
                    <div className="flex items-center gap-3 mb-6">
                      ...
                      <h2 className="text-xs font-black uppercase tracking-[0.2em] text-gray-600">Pinned Notes</h2>
                    </div>
                ...
                <section>
                  <div className="flex items-center gap-3 mb-6">
                    ...
                    <h2 className="text-xs font-black uppercase tracking-[0.2em] text-gray-600">
                      {pinnedNotes.length > 0 ? 'Recent' : 'All Notes'}
                    </h2>
                  </div>
                  
                  {regularNotes.length === 0 && pinnedNotes.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-24 text-center">
                      <div className="w-24 h-24 bg-white/5 rounded-[2rem] flex items-center justify-center mb-6 border border-white/10 rotate-12 group hover:rotate-0 transition-transform duration-500">
                        <i className="fas fa-feather text-4xl text-gray-700 group-hover:text-indigo-500 transition-colors"></i>
                      </div>
                      <h3 className="text-xl font-bold text-white mb-2">No notes found</h3>
                      <p className="text-gray-500 max-w-xs mb-8">Every great idea starts with a single word. Start yours today.</p>
                      <button 
                        onClick={handleCreateNote}
                        className="px-8 py-3 bg-indigo-600 hover:bg-indigo-700 rounded-2xl text-sm font-black transition-all shadow-xl shadow-indigo-600/20 active:scale-95"
                      >
            ...
            <button 
              onClick={handleCreateNote}
              className="fixed bottom-10 right-10 w-16 h-16 bg-indigo-600 rounded-3xl flex items-center justify-center shadow-2xl shadow-indigo-600/40 hover:scale-110 hover:-rotate-6 active:scale-95 transition-all z-40 group"
            >
